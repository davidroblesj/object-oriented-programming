...
public class LevelGeneration : MonoBehaviour
{
	...
	public void DrawMap()
	{
		foreach (Room room in rooms)
		{
			...
			MapSpriteSelector mapper = Object.Instantiate(roomWhiteObj, drawPos, Quaternion.identity,mapa).GetComponent<MapSpriteSelector>();
	...
	public void DrawMap(Room laroom,bool state)
	{
		foreach (Room room in rooms)
		{
			if (room == null)
			...
			if (laroom == room)
			{
				...
				laroom.type = 1;
					...
					laroom.type = 0;
				Vector2 drawPos = laroom.gridPos;
				drawPos.x *= 16;//aspect ratio of map sprite
				drawPos.y *= 8;
				//create map obj and assign its variables
				MapSpriteSelector mapper = Object.Instantiate(roomWhiteObj, drawPos, Quaternion.identity, mapa).GetComponent<MapSpriteSelector>();
				mapper.type = laroom.type;
				mapper.up = laroom.doorTop;
				mapper.down = laroom.doorBot;
				mapper.right = laroom.doorRight;
				mapper.left = room.doorLeft;
			}
		}
	}
	public void UpdateMap(Room laRoom,bool state)
	{
		foreach (Room room in rooms)
		{
